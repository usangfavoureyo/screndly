...
interface JobsState {
  ...
  // Job management
  addJob: (job: Omit<UploadJob, 'id' | 'createdAt' | 'updatedAt' | 'events'>) => Promise<string>;
  ...
  persist(
    (set, get) => ({
      ...
      duplicateJob: (id) => {
        ...
        get().addSystemLog({
          severity: 'info',
          message: `Job duplication requested: ${job.fileName}`,
        ...
        pollingInterval = setInterval(async () => {
          try {
            ...
            if (response.success && response.data) {
              ...
              const updatedJobs = data.map((fetchedJob: any) => {
                const existing = currentJobs.find(j => j.id === fetchedJob.id);
                if (existing && existing.stage !== fetchedJob.stage) {
                  get().addJobEvent(fetchedJob.id, {
                    severity: 'info',
                    message: `Stage changed to ${fetchedJob.stage}`,
                  });
                }
                return fetchedJob;
              });
