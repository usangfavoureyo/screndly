...
router.post('/callback', async (req, res) => {
    try {
        const { platform, code, redirectUri, state } = req.body;
        ...
        if (!decodedState) {
            return res.status(401).json({
                success: false,
                error: { message: 'Valid OAuth state is required for callback exchange.' }
            });
        }
        ...
        if (normalizedPlatform === 'Instagram' || normalizedPlatform === 'Facebook') {
            ...
            if (!decodedState?.codeVerifier) throw new Error('Missing PKCE verifier for YouTube OAuth');
            assertConfigured('YouTube', {
                YOUTUBE_CLIENT_ID: env.YOUTUBE_CLIENT_ID,
                YOUTUBE_CLIENT_SECRET: env.YOUTUBE_CLIENT_SECRET
            });
