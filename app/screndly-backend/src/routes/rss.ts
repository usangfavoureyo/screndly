...
router.get('/feeds', async (_req: Request, res: Response) => {
  try {
    ...
    const transformedFeeds = feeds.map((feed) => ({
      ...
      openaiWebSearchEnabled: (feed as any).openaiWebSearchEnabled ?? false,
      serperPriority: feed.serperPriority,
      imageSourcePriority: (feed as any).imageSourcePriority ?? (feed.serperPriority ? 'serper_first' : 'tmdb_first'),
