...
iframe.addEventListener('load', () => {
  ...
  setTimeout(() => {
    console.log('posting echo script');
    iframe.contentWindow.postMessage('app.echoToOE("ping");', '*');
  }, 10000);
});
</script></body></html>`;
const server = http.createServer((req, res) => { res.writeHead(200, {'Content-Type':'text/html'}); res.end(html); });
server.listen(43125, '127.0.0.1', async () => {
  ...
  await page.goto('http://127.0.0.1:43125', { waitUntil: 'load', timeout: 120000 });
